		    		...
		    		for (Player all : plugin.getServer().getOnlinePlayers())
			        	if(killer.getItemInHand().hasItemMeta()) { //!killer.getItemInHand().getItemMeta().hasDisplayName()
			        		if(!killer.getItemInHand().getItemMeta().hasDisplayName()) {
			        			all.sendMessage("§a"+killer.getName()+"§e matou §c"+p.getName()+"§e e dropou a cabeça");
			        		}else{
			        			all.sendMessage("§a"+killer.getName()+"§e matou §c"+p.getName()+"§e usando "+(killer.getItemInHand().getItemMeta().getDisplayName().replaceAll("	", " ").replaceAll("  ", " "))+"§e e dropou a cabeça");
			        		}
			        	}else{
			        		all.sendMessage("§a"+killer.getName()+"§e matou §c"+p.getName()+"§e e dropou a cabeça");
			        	}
